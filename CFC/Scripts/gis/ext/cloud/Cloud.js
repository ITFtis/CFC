...
(function ($) {
    ...
    $.cloud = {
        eventKeys: { type_change: "cloud_type_change", init_ui_completed: "init_ui_completed", add_Image_completed: "add_Image_completed" },
        ...
        initUi: function () {
            ...
            try {
                $.each(this.settings.clouds, function (idx, item) {
                    var gdom = $('<div class="cloud-group col-xs-12 col-12 row auto-append-header" data-header=' + item.name + '></div>');//ie color會被override掉(所以加style),hover也不work
