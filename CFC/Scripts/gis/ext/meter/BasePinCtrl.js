...
(function ($) {
    ...
    pluginclass.prototype = {
        ...
        repaintLegendUI: function (_legendIcons) {
            var current = this;
            var $_lui = $(".legend", this.$element);
            if ($_lui.length == 0) {
                //this.$element.append('<div class="col-xs-12 col-12 legend">'); //remark 20190904
                this.$element.append('<div class="legend">');
                $_lui =  $(".legend", this.$element);
            }
            $_lui.empty();
            if (_legendIcons)
            ...
            if (this.settings.legendIcons) {
                $.each(this.settings.legendIcons, function (idx, icon) {
                    if (current.settings.type == $.BasePinCtrl.type.point) {
                        var cls = 'legend-icon ';
                        var $_dom = $('<div>' + icon.name + '</div>').addClass('legend-icon' + (icon.disabled ?' legend-disable default-disabled':'' ));
                        $_lui.append($_dom);
                        ...
                        $('<svg width="26px" height="14px"><g><path></path></g></svg>').appendTo($_c).find('path').attr('stroke', this.strokeColor).attr('stroke-opacity', this.strokeOpacity).attr('stroke-width', this.strokeWeight)//this.strokeWeight)
                        ...
                        var $_c = $('<span class="legend-icon' + (icon.disabled ? ' legend-disable default-disabled' : '') +'"><div></div><span class="legend-name">' + icon.name + '</span></span>').appendTo($_lui).find('> div').css('display', 'inline-flex').css('vertical-align', 'sub');
                        $('<svg width="26px" height="14px"><g><path></path></g></svg>').appendTo($_c).find('path').attr('stroke', this.strokeColor).attr('stroke-opacity', 1).attr('stroke-width',  3)//this.strokeWeight)
        ...
        show: function () {
            ...
            this.$element.trigger($.BasePinCtrl.eventKeys.pinShowChange, [true]);
        },
        hide: function () {
