...
(function ($) {
    ...
    if (!$.AppConfigOptions.script.gispath)
    ...
    var __initGisMenu = function (menu) {
        ...
        var mainmenu_height = mainmenu.outerHeight ? mainmenu.outerHeight() : mainmenu.height();
        ...
        var $popuCtrlMenu = $(".popu-ctrl-menu").on('click', function (evt) {
            ...
            if (ctrl_content.length > 0 && $(this).attr('popu-target') == undefined) {
                ...
                var jp = $(".popu-ctrl-container").jsPanel({
                    ...
                    position: $.isSupportLocalStorage() ? { top: evt.clientY, left: evt.clientX } : { top: mainmenu_height, left: isDesktop() ? $(".popu-ctrl-container .jsPanel").length * 15 : "" },// $('#' + menuid).height() },// bottom: 100, right: 70 },
                ...
                jp.on('dragstop', function () {
                    //var ctop = $(item).attr('data-top',jp.position().top);
                    //var cleft = $(item).attr('data-left', jp.position().left);
                ...
                if ($(this).attr("data-default-top")) //cache top
                    jp.attr("data-top", $(this).attr("data-default-top") + "px");
                ...
                if ($(this).attr("data-default-left")) //cache top
                    jp.attr("data-left", $(this).attr("data-default-left") + "px");
        ...
        var $positCtrlMenu = $(".posit-ctrl-menu").on('click', function (evt) {
            ...
            var _dir = ctrl_content.attr('data-display-direction');
            if (_cache_show != 'false')
                setLocalCache($current, { show: !ctrl_content.is(":visible") }); //cache show 的狀態
            
            if ('up-down' === _dir) {
                if (ctrl_content.is(":hidden"))
                    ctrl_content.slideDown(500);
                else
                    ctrl_content.slideUp(500);
            }
            else
                ctrl_content.toggle('slide', { direction: _dir ? _dir : 'left', easing: "easeInOutBack" }, 500);
        ...
        var popuSwitch = function ($trigglea, $popu) {
            ...
            var animationClass = isSwitchPopu ? 'switch' : 'popu-slide';
            ...
            return;//20200513改非isDeskTop 不用$popu.toggle("slide", { direction: 'left', duration: 300, easing: "easeOutQuad" }, displayJspanel);
            if (isDesktop() && $.isSupportLocalStorage()) {
                $popu.show();
                //"jsPanel-min-container"
                if ($popu.parents("#jsPanel-min-container").length == 0) { //非最小化
                    ...
                    $(".jsPanel-hdr-r-btn-max", $popu).trigger("click");
                $popu.addClass("switch").toggleClass("selected");//要先加switch 在加 selected才會有效果
                ...
                //$popu.toggle("slide", { direction: 'left', duration: 300, easing: "easeOutQuad" }, displayJspanel);//會觸發jqueryj會在dom外包一個<div class="ui-effects-wrapper">所以觸發 DOMNodeRemoved 及 onjspanelclosed event
                //$popu.toggleClass("selected");
                $popu.addClass("popu-slide");
                $popu.show();
                setTimeout(function () {
                    $popu.toggleClass("selected");
                    $popu.one('transitionend oTransitionEnd otransitionend MSTransitionEnd',
                    ...
                    if (_cache_show != 'false')
                        setLocalCache($trigglea, { show: !$popu.is(":visible") }); //cache show 的狀態
                });
            }
            
            $trigglea.trigger($.menuctrl.eventKeys.ctrl_show_change, _show);
            //triggerShowGroup($trigglea);
        };
                ...
                if (isDesktop()) { //一般layout
                    $.each(popu_ctrls, function (idx, item) {
                        ...
                        if ($item.attr('data-top') !== undefined) {
                            ...
                            if (window.outerHeight - ctop < 70) ctop = window.outerHeight - 70;
                            if (window.outerWidth - cleft < 200) cleft = window.outerWidth - 200;
                            $(item).attr('data-top', "");
                            $(item).attr('data-left', "");
                        ...
                        if ($("").draggable && $item.hasClass('horizontal-drag-remove')) 
                            $item.draggable("option", "axis", false).off('dragstop').removeClass('horizontal-drag-remove');
                    ...
                    $.each(popu_ctrls, function (idx, item) {
                        ...
                        if ($item.css('position') != 'relative') {
                            ...
                            if (($item.attr("style") + "").indexOf("width:") >= 0) { //無width style是用bootstrape的col-md-2
                            ...
                            $item.off('dragstart').on('dragstart', function () {
                                ...
                                popu_ctrl_container.addClass('width_100');
        ...
        var setLocalCache = function (jquerytarget, cvalue) { //jquerytarget是選單功能的<a>
            $.localCache.set(jquerytarget.text() ? jquerytarget.text() : jquerytarget.attr('title'), $.extend($.localCache.get(jquerytarget), cvalue));
        };
            ...
            mainmenu.find('.popu-ctrl-menu').each(function(){
                ...
                $($_mctrl.attr('href')).on($.menuctrl.eventKeys.popu_init_after, function () {
                    ...
                    if (isFixedSingle) {
                        ...
                        if (jphs.backgroundColor == 'transparent' || jphs.backgroundColor == 'rgba(0, 0, 0, 0)')
    ...
    $.fn.gis_layer_opacity_slider = function (options) {
        ...
        function setOpacity() { //google的Opacity由各Overlays，無法由layer統一控制
            if (settings.setOpacity)
                settings.setOpacity(current.slider("value") / 100);
            if (hasLayer) {
                settings.map.getLayer(settings.layerid).setOpacity(current.slider("value") / 100);
            }
        };
        setOpacity();
