...
var jspAlertMsg = function (jcontainer, optins, callback) {
    ...
    var $_header = $('<div class="modal-header"><h4 class="modal-title pull-left float-left" >' + settings.title + '</h4><button type="button" class="close pull-right float-right" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button></div>').appendTo($_content);
...
var jspConfirmYesNo = function (jcontainer, optins, callback) {
    ...
    if ($_bv && $_bv.startsWith('5'))
        $_header = $('<div class="modal-header"><h4 class="modal-title" id="myModalLabel">' + settings.title + '</h4><button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button></div>').appendTo($_content);
    else
    ...
    var $_closebtn = $('<button type="button" class="btn btn-default btn-outline-dark  取 消  btn-sm" data-dismiss="modal"> 取 消 </button>').appendTo($_footer);
...
(function () {
    ...
    var genBootstrapAccordion = function ($_container, _id, _classes, _heads, _contents) {
        ...
        for (var _idx = 0; _idx < _heads.length; _idx++) {
            ...
            $('<div class="panel-heading" role="tab"><h4 class="panel-title"><a role="button" data-toggle="collapse" data-parent="#' + id + '" href="#content' + _idx + '-' + id + '" aria-expanded="false" aria-controls="content' + _idx + '-' + id + '">' + _heads[_idx] + '</a></h4></div>').appendTo($_panel);

            $('<div id="content'+_idx+'-' + id + '" class="panel-collapse collapse '+(_idx==0?'in':'')+'" role="tabpanel" aria-labelledby="content-' + id + '"><div class="panel-body">'+_contents[_idx]+'</div></div>').appendTo($_panel);
        }
        if ($_container)
    ...
    var genBootstrapCarousel = function ($_container, _id, _classes, _contents, _contentCaptions) {
        ...
        var $_carousel = $('<div id="' + id + '" class="carousel slide '+(_classes?_classes:'')+'" data-ride="carousel">');
        ...
        $.each(_contents, function (idx, c) {
            $('<li data-target="#' + id + '" data-slide-to="' + idx + '" class="' + (idx == 0 ? 'active' : '') + '"></li>').appendTo($_ol)
        ...
        $('<a class="carousel-control-prev left carousel-control" href="#' + id + '" role="button" data-slide="prev"><span class="glyphicon glyphicon-chevron-left" aria-hidden="true"></span><span class="sr-only">Previous</span></a>').appendTo($_carousel);
        $('<a class="carousel-control-next right carousel-control" href="#' + id + '" role="button" data-slide="next"><span class="glyphicon glyphicon-chevron-right" aria-hidden="true"></span><span class="sr-only">Next</span></a>').appendTo($_carousel);
