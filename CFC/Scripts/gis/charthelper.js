(function () {
    ...
    var showMeterChart = function ($_c, st, ddatas, title, yAxistitle, yAxismin, serdefs, beforePainChart, mouseIndexChageCallback) {
        ...
        var $t = $('<div id="chart-' + _id + '">').appendTo($_c.empty());
        var _chartoptions = {
            chart: {
                //animation:false,
                renderTo: 'chart-' + _id,
            ...
            tooltip: {
                ...
                formatter: function (evt) {
                    ...
                    $.each(this.points, function () {
                        var thisp = this;
                        var _as = $.grep(_alerts, function (s) { return thisp.series.name == s.name });
                        if (_as.length > 0)
                            return;
                        //if (!(this.series.name == _info.name ))
                        //    return false;
                        ...
                        events: {
                            mouseOver: function () {
                                if (mouseIndexChageCallback)
                                    mouseIndexChageCallback(this.index);
                            }
                        }
                    }
                },
        ...
        //水波#1E90FF'#ADD8E6'
        var ic = Highcharts.Color(_info.color);//.setOpacity(.2).get('rgba'));
        ...
        //}, 500);
        if (_info && _info.data.length > 1000 && !_info.turboThreshold)//turboThreshold預設1000，超過整個series就不會呈現
            console.log("_info.turboThreshold 最大限制1000，目前資料: " + _info.data.length+" ***************")
        return chart;
    }
